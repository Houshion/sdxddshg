...
class RoleController extends BaseController
{
	...
	public function authDetail(){
        ...
        if(request()->isPost()) {
            ...
            if($data['oath_id'] = I('post.oath_id')) { //修改
                ...
                $Dlc_auth = model('role') -> where(array('role_id' => 1)) -> getField('auth');
                model('role') -> save(array('role_id' => 1,'auth' => $Dlc_auth.",".$a));
            }
            if($a) {
        ...
        $oath1 = model('roleOath')->where(array('module' => 'Dlc','is_menu' => 1,'parent_id' => 0))-> order(array('order' => 'desc')) -> select();
        $oath2 = model('roleOath')->where(array('module' => 'Dlc','parent_id' => array('NEQ',0)))-> order(array('is_menu' => 'desc','order' => 'desc')) -> select();
    ...
    */
	public function roleDetail(){
        ...
        $oath1 = model('role_oath')->where(array('module' => 'Dlc','is_menu' => 1,'parent_id' => 0))->order(array('order' => 'desc'))->select();
        $oath2 = model('role_oath')->where(array('module' => 'Dlc','parent_id' => array('NEQ',0)))->order(array('is_menu' => 'desc','order' => 'desc'))->select();
        $this->assign('oath1', $oath1);
        $this->assign('oath2', $oath2);
    ...
    */
    public function authEdit(){
        ...
        $cache = $m->alias('a')->join('dlc_role b ','a.roleid = b.role_id')->field('a.*,b.role_name')->where($map)->page($p, $psize)->select();
        $count = $m->alias('a')->join('dlc_role b ','a.roleid = b.role_id')->field('a.*,b.role_name')->where($map)->count();
    ...
    */
    ...
    public function roleDel(){
        ...
        $auth = model('role')->where(['role_id'=>$role_id])->value('auth');
        $auths = explode(',',$auth);
        if($auths[2] != ''){
            $info['status'] = 0;
            $info['msg'] = '该角色下还有未取消的权限,请先取消再删除！';
            return($info);
        }
        $re = model('role')->where(['role_id'=>$role_id])->delete();
        if($re){
            $info['status'] = 1;
            $info['msg'] = '删除成功！';
            return($info);
