...
class Recharge extends Model
{
    ...
    public function payOk($transaction_id,$out_trade_no,$pay_ment = 1,$money=0)
    {
        ...
        if($recharge_order){//充值订单
            ...
            $cash_log['money']		  = $recharge_order['money']+$recharge_order['give_money'];
            $cash_log['type']		  = 2;
            $cash_log['pay_type']     = $pay_ment;
            $cash_log['ctime']		  = time();
            $cash_log['shop_id']		  = $recharge_order['shop_id'];
            $res1 = Db::name('cash_log')->insert($cash_log);
           $user_money= db('user_money')->where(['shop_id'=>$recharge_order['shop_id'],'user_id'=>$recharge_order['user_id']])->find();
//    		//修改用户余额
            $data['user_id']=$recharge_order['user_id'];
            $data['shop_id']=$recharge_order['shop_id'];
            $data['user_money']=$user_money['user_money'] + $recharge_order['money']+$recharge_order['give_money'];
            $res2= db('user_money')->insert($data);
            //修改充值订单状态
            $res3 = Db::name('recharge')->where(['order_number'=>$out_trade_no])->update(['status'=>2,'pay_time'=>time()]);
