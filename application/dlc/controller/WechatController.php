...
class WechatController extends Controller
{
    ...
    public function init()
    {
        ...
        $config = db('wxpay')->find();

        $options = array(
            'token' => $config ["wxmchkey"], //填写你设定的key
    ...
    public function checkEvents($event)
    {
        ...
        switch ($event) {
            case 'subscribe':
                ...
                $this->checkChannel($openId,'subscribe');
                $this->checkKeyWords('subscribe');
                break;
            case 'unsubscribe':
                $this->updateUser($openId);
                $this->checkChannel($openId,'unsubscribe');
                break;
            case 'CLICK':
    ...
    //渠道检查
    public function checkChannel($openId = '',$type = 'subscribe')
    {
        $channel_id = self::$weObj->getRevSceneId();

        $params['type'] = $type;
        $params['openid'] = $openId;
        $params['channel_id'] = $channel_id;
        if(class_exists('addons\\common\\channel\\Channel')){
            \think\Hook::exec('addons\\common\\channel\\Channel','checkChannel',$params);
        }
    }
    ...
    public function checkKeyWords($key = '')
    {
        ...
        $reply = model('WxReply')->with('file')->where('key',$key)->find();
        if ($reply) {
            if ($reply["type"] == "news") {
                $newsArr = array(
                    array(
                        ...
                        'PicUrl' => self::$appUrl . '/public/uploads/' . $reply['file']["savepath"] . $reply['file']["savename"],
                        'Url' => $reply["url"]
                    )
                );
    ...
    //新订单通知
    public function sendTplMsgOrder($order_number='')
    {
        write_log('sendTplMsgOrder','开始推送2');
        $this->init();
        $order =db('order')->alias('a')
            ->join('dlc_user b','a.user_id=b.user_id')
            ->join('dlc_order_info c','c.order_id=a.order_id')
            ->join('dlc_goods d','d.goods_id=c.goods_id')
            ->field('a.order_id,a.order_number,a.pay_price,a.pay_time,b.openid,d.title,a.num')
            ->where(array('a.order_number'=>$order_number))
            ->select();
        $title=[];
        $num=0;
        $pay_price=0;
        ...
        $meesage = [
            'touser' => $openid,
            'template_id' => '4LhngTo2BVZSwl4FR--LvxZ3OdDUX21_DSdJY6-Vtk4',
            'url' => '',
            'data' => [
                'first' => [
                    'value' => '恭喜你购买成功!',
                ...
                'keyword4' => [
                    'value' => date('Y-m-d H:i',$pay_time),
                    'color' => '#173177',
                ],
                'remark'=>[
                    'value' => '欢迎使用华威科自动售货机购物,祝您生活愉快！',
                    'color' => '#173177',
                ]
...
//        die;
        //if($tempMsg['status']){
        write_log('sendTplMsgOrder','开始推送3'.'-----order_id---'.$order['order_id']);
            self::$weObj->sendTemplateMessage($meesage);
        write_log('sendTplMsgOrder','开始推送5');
//        }
        // $this->sendTplMessageOrderAdmin($order_id);
    }
    //优惠券通知
    public function sendTplMsgCoupon($coupon_id='')
    {
        $this->init();
        $title=[];
        $num=0;
        $openid='';
        $meesage = [
            'touser' => 'oRW6m1AGov9XXOkYA9ZW4yZFIU2M',
            'template_id' => 'EalxdVtwNbPOA4PbKzfJ-33mbq8yFph4WygLdqROf3w',
            'url' => '',
            'data' => [
                'first' => [
                    'value' => '您好！你有张新的优惠券请注意查看！',
                    'color' => '#173177',
                ],
                'name' => [
                    'value' => '您的有新有优惠券',
