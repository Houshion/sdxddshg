...
class BaseController extends Controller
{
    ...
    protected function _initialize()
    {
        ...
        if(!in_array($action, $other)){
           $this ->uid = Session::get('MMS.uid');//必须保证有Userid，去掉登录功能模块↑
           $this ->user = Session::get('MMS.user');//必须保证有Userid，去掉登录功能模块↑
           $this ->type = Session::get('MMS.type');//必须保证有Userid，去掉登录功能模块↑
           $this ->shop_id = Session::get('MMS.shop_id');//必须保证有Userid，去掉登录功能模块↑
           if($this->type == 1){
                $role_id = Db::name('admin') -> where(array('id' => $this ->uid))->value('roleid');
           }else{
                $role_id = Db::name('user') -> where(array('user_id' => $this ->uid))->value('roleid');
           }

           $this->role_id = $role_id;
            $this->shop_id=db('shop')->where('aid',$this ->uid)->value('shop_id');
