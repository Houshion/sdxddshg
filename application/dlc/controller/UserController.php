...
{
    public $url = '';
    public $authStatus = array(0=>'--',1=>'<font color="#53a93f">审核中</font>',2=>'<font color="#53a93f">已认证</font>',3=>'<font color="#53a93f">认证失败</font>');
...
//        $psize =1;
        ...
        foreach ($result as $k=>$v){
			...
			if(empty($v['nickname']))$result[$k]['nickname'] = "--";
			if($v['mobile'] == 0)$result[$k]['mobile'] = "--";
			//用户消费金额
			$wap["user_id"] = $v["user_id"];
			$wap["status"] = 3;
            $result[$k]["consume_money"] = Db::name("order")->where($wap)->sum('pay_price');

//			die;
			if(empty($result[$k]["consume_money"]))$result[$k]["consume_money"] = '0.00';
			//用户抵押金额
			$wap1["user_id"] = $v["user_id"];
			$wap1["discount_id"] = array("NEQ",0);
            $result[$k]["discount_money"]   = Db::name("order")->where($wap1)->sum("discount_money");

			if(empty($result[$k]["discount_money"]))$result[$k]["discount_money"] = '0.00';
...
//        print_r($result);
    ...
    public function edit(){
        ...
        $bread = array(
            ...
            '1' => array(
                ...
                'url' => url('dlc/User/edit?user_id='.$user_id), 
            ), 
        );
        $this->assign('breadhtml', $this->getBread($bread));
    ...
    public function userDiscounts()
    {
        ...
        $bread = array(
            ...
            '2' => array(
                'name' => '用户优惠列表',
                'url' => url('dlc/User/userDiscounts'), 
            ), 
        );
        $this->assign('breadhtml', $this->getBread($bread));
        $status = input('status');
    ...
    public function order()
    {
        ...
        $field= ['a.*,b.address,c.user_name,d.macno,e.nickname'];
        $order= 'a.ctime desc';
        $money=0.00;
        ...
        $this->getPage($count, $size, 'App-loader','用户订单列表', 'App-search');
...
/************************* 商家列表开始 **************************/
    ...
    { 
        ...
        $result = Db::name('shop')->alias('a')
                  ->join('dlc_admin b','b.id=a.aid')
                  ->where($map)->order('a.ctime desc')
                  ->field($field)
                  ->page($page,$size)->select();
        foreach ($result as $key => $value) {
            $result[$key]['device_num'] = Db::name('device')->where(['shop_id'=>$value['shop_id']])->count() ? : 0;
            $result[$key]['user_num']  = Db::name('user')->where(['shop_id'=>$value['shop_id']])->count() ? : 0;
            $result[$key]['discount_money']= Db::name('order')->where(['shop_id'=>$value['shop_id'],'status'=>3])->sum('discount_money') ? : '0.00';
            $result[$key]['total_price']= Db::name('order')->where(['shop_id'=>$value['shop_id'],'status'=>3])->sum('total_price') ? : '0.00';
            $result[$key]['pay_price']= Db::name('order')->where(['shop_id'=>$value['shop_id'],'status'=>3])->sum('pay_price') ? : '0.00';
        }
        ...
        $this->assign('empty','<tr><td colspan="14" style="line-height:32px;text-align:center;">暂无数据！</td></tr>');
        echo $this->fetch();
    }
    ...
    public function shopSet()
    {
        ...
        $bread = array(
            ...
            '1' => array(
                'name' => $id ? '编辑商家':'新增商家',
                'url' => url('dlc/User/shopSet'), 
            ), 
        );
        if($_POST){
            $post = input('post.');
            if($id){
                $sfind = Db::name('shop')->where(['shop_id'=>['neq',$id],'user_phone'=>$post['account']])->find();
                $afind = Db::name('admin')->where(['id'=>['neq',$post['aid']],'username'=>$post['username']])->find();
                if(!empty($sfind) || !empty($afind)){
                    return(['status'=>0,'msg'=>'此帐号已被注册过了']);
                }
                $data1['user_name'] = $post['user_name'];
                $data1['user_phone']= $post['user_phone'];
                $data1['account']   = $post['account'];
                $data1['memo']      = $post['memo'];
                $data1['scale']      = $post['scale'];
                if($post['password2'] != ''){
                ...
                $sfind = Db::name('shop')->where(['user_phone'=>$post['account']])->find();
                $afind = Db::name('admin')->where(['username'=>$post['username']])->find();
                if(!empty($sfind) || !empty($afind)){
                    return(['status'=>0,'msg'=>'此帐号已被注册过了']);
                }
                $bool= $this->check_mobile($post['account']);
                if(empty($bool)){
                    return(['status'=>0,'msg'=>'移动端帐号输入不正确，请输入正确的手机号']);
                }
                $bool1= $this->check_mobile($post['user_phone']);
                if(empty($bool1)){
                    return(['status'=>0,'msg'=>'联系方式输入不正确，请输入正确的手机号']);
                }
        ...
        $result = Db::name('shop')->alias('a')
                  ->join('dlc_admin b','b.id=a.aid','left')
    ...
    public function shopDevice()
    {
        ...
        $bread = array(
           '0' => array(
            ...
            '1' => array(
                'name' => $type==1 ? '商家列表' : '员工列表' ,
                'url' =>  $type==1 ? url('dlc/User/shop') : url('dlc/User/staff'), 
            ), 
            '2' => array(
                'name' => $type==1 ?'商家设备列表':'员工设备列表',
                'url' => url('dlc/User/shopDevice'), 
            ), 
        );
        $title  = input('title');
        $macno  = input('macno');
        ...
        if($type==1){
           ...
           $name = '商家设备列表'; 
            ...
            $shop_id = Db::name('user')->where(['user_id'=>$id])->value('shop_id');
...
//            ->join('dlc_device_goods b','b.goods_id=a.goods_id','left')
    ...
    public function apply()
    {
        $bread = array(
            ...
            '1' => array(
                'name' => '加盟申请列表',
                'url' => url('dlc/User/apply'), 
            ), 
        );
        $username =trim( input('username'));
        $user_name= trim(input('user_name'));
        if($username){
            $map['b.nickname|a.user_name'] = ['like','%'.$username.'%'];
            $this->assign('username',$username);
        }
//        if($user_name){
//            $map['a.user_name'] = ['like','%'.$user_name.'%'];
...
//        }
        ...
        $result = Db::name('cooperation')->alias('a')
                  ->join('dlc_user b','b.user_id=a.user_id','left')
                  ->where($map)->page($page,$size)->field('a.*,b.nickname')
...
/************************* 员工列表开始 **************************/
    ...
    public function staff()
    {
        ...
        $count  = Db::name('user')->alias('a')
                ->join('dlc_shop b','b.shop_id=a.shop_id','left')->where($map)->order('a.ctime desc')->count();
        $shop  = Db::name('shop')->select();
        $this->assign('shop',$shop);
        $this->assign('result',$result);
        $this->assign('breadhtml', $this->getBread($bread));
...
//                dump($this->check_mobile($post['mobile']));
               $bool= $this->check_mobile($post['mobile']);
                ...
                $find = Db::name('user')->where(['user_id'=>['neq',$id],'mobile'=>$post['mobile']])->find();
                if($find){
                    $info['status'] = 0;
                    $info['msg']    = '此帐号已被注册过了';
                    return($info);
...
//        $psize =1;
        ...
        $result =db('device_open_log')->alias('a')
                 ->join('dlc_user b','b.user_id=a.user_id','left')
                 ->join('dlc_device c','c.device_id=a.device_id','left')
                 ->join('dlc_shop d','c.shop_id=a.shop_id','left')
                  ->field('a.*,b.nickname,b.mobile,c.macno,d.user_name')
