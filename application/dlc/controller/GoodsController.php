...
//        print_r($this->uid);
        $pagesize =self::$CMS['set']['pagesize']?:20;
        $page = input('page')?input('page'):1;
        $result = model('goods')->get_list($condition,'*',$page,$pagesize);
        $result = $result->toArray();
//        $result = db('goods')->alias('a')
//            ->join('dlc_goods_time b','a.goods_id=b.goods_id','left')->where($condition)->field('a.*,b.sale')->page($page, $pagesize)->select();

        foreach ($result as $k=>$v){
            $goodsType = model('goodsType')->get_row(['type_id'=>$v['type_id']]);
            $result[$k]['type'] = $goodsType['type'];
            $result[$k]['img'] ='http://'.$_SERVER['HTTP_HOST'].'/public/'.$v['img'];
			$result[$k]["unuse"]  = Db::name("rfid")->where("goods_id",$v["goods_id"])->where("status",1)->count();  //未使用
			$result[$k]["onsale"] = Db::name("rfid")->where("goods_id",$v["goods_id"])->where("status",2)->count();  //在售
			$result[$k]["sold"]   = Db::name("order a")
                                    ->join('dlc_order_info b','b.order_id=a.order_id','left')
                                    ->where("b.goods_id",$v["goods_id"])->where("a.status",3)
                                    ->sum('b.num') ? : 0;  //已出售
			
        }
        $count = db('goods')->where($condition)->count();
        ...
        if ($post){ //有交互数据
            if ($goods_id){ //修改
                $goodsData = array(
                    ...
                    'unit'      =>  $post['unit'],
                    'cost'      =>  $post['cost'],
//					'webrate'   =>  $post['webrate'],
//					'staffrate' =>  $post['staffrate'],
                );
...
//					'staffrate' =>  $post['staffrate'],
                ...
                if ($post['rfid']){
                    $rfidArr = explode(',',$post['rfid']);
                    foreach ($rfidArr as $k=>$v){
						$is_has = Db::name('rfid')->where('rfid',$v)->find();
						if(!empty($is_has))continue;
                        $rfidData = array(
                            'rfid'      =>  $v,
                            'goods_id'  =>  $goodsId,
                            'ctime'     =>  time(),
                        ...
                        Db::name('rfid')->insert($rfidData,true);
                    }
                    $data['goods_id']=$goods_id;
	...
	public function rfid(){
        ...
        $bread = array(
        	'0' => array(
            	'name' => '商品管理',
                'url' => url('dlc/goods/index'),
             ),
             '1' => array(
                 'name' => 'rfid列表',
                 'url' => url('dlc/goods/rfid')
             ),
        );
        $this->assign('breadhtml', $this->getBread($bread));
		$pagesize = 10;
		...
		$list = Db::name('rfid')
			->alias('a')
			->join("dlc_goods b","a.goods_id = b.goods_id")
			...
			->group('a.rfid')
			->select();
        $count = Db::name('rfid')->where("goods_id",$goods_id)->count();
	...
	public function rfidlist(){
        ...
        $rfids = trim(input('rfid'));
        $goodsname = trim(input('goodsname'));
        $devicename = trim(input('devicename'));
		if ($rfids){
            $where['a.rfid'] = ['like','%'.$rfids.'%'];
            $this->assign('rfid',$rfids);
        }
        if ($goodsname){
		...
		$list = Db::name('rfid')
			...
			->join("dlc_device c","a.device_id = c.device_id","left")
			->join("dlc_shop d","a.shop_id = d.shop_id","left")
			->where($where)
			->field("a.*,b.title as goodsname,c.title as devicename,d.user_name as shopname")
			->page($page,$pagesize)
			->group('a.rfid')
			->select();
//		echo Db::name('rfid')->getLastSql();
        $count = Db::name('rfid')->alias('a')->alias('a')
            ->join("dlc_goods b","a.goods_id = b.goods_id","left")
            ->join("dlc_device c","a.device_id = c.device_id","left")
            ->join("dlc_shop d","a.shop_id = d.shop_id","left")
            ->where($where)->count();
	...
	public function importRfid(){
		$goods_id = input("goods_id");
		$this->assign('goods_id',$goods_id);
		echo $this->fetch('importRfid');
	}
	
	//保存rfid
	public function rfidSave(){
		$goods_id = input("goods_id");
		if(empty($goods_id))$goods_id = 0;
		$list = explode(",",input("rfid"));
		if(empty($list)){
			$info['status'] = 0;
            $info['msg'] = 'rfid不能为空';
			return($info);
		}
		$batch = Db::name("rfid")->max("batch");
		foreach($list as $k=>$v){
			$rfid[$k]["rfid"] = trim($v);
			$rfid[$k]["goods_id"] = $goods_id;
			$rfid[$k]["batch"] = ($batch+1);
			$rfid[$k]["ctime"] = time();
			$re = Db::name("rfid")->insert($rfid[$k],true);
		}
		if ($re !== false){
        	$info['status'] = 1;
            $info['msg'] = '上传成功';
...
//			$data['status'] = input("status");
			...
			if(!empty($where)){
				$re = Db::name("rfid")->where($where)->update($data); 
				if ($re !== false){
					$info['status'] = 1;
					$info['msg'] = '保存成功';
				}else{
...
//                    'end_time'  =>  strtotime($post['end_time']),
                ...
                $result = db('goodsType')->where(['type_id'=>$type_id])->update($typeData);
                if ($result){
                    $info['status'] = 1;
                    $info['msg'] = '修改成功';
                }else{
...
//                    'end_time'  =>  strtotime($post['end_time']),
            ...
            if ($type_id){ //修改模板
                $result = db('goodsType')->where(['type_id'=>$type_id])->find();
//                if($result['start_time']==0){
//                    $start_time= '';
//                }else{
//                    $start_time= date('Y-m-d H:i:s',$result['start_time']);
//                }
...
//        $map['shop_id']=$this->shop_id;
        $lsit=db('shop_time')->alias('a')
            ->join('dlc_shop b','a.shop_id=b.shop_id','left')
            ->where($map)
            ->field('a.*,b.user_name')
...
//        }
    ...
    public function goodssalelist(){
        ...
        if($user_name){
            $map['d.user_name']=['like',"%$user_name%"];
            $this->assign('user_name',$user_name);
        }
...
//        $map['a.shop_id']=$this->shop_id;
        $lsit=db('goods_time')->alias('a')
            ...
            ->field('a.*,b.title,c.start_time,c.end_time,d.user_name')
            ->where($map)
            ->page($page,$pagesize)->select();
